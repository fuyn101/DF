function export_results_to_csv(valid_fractal_info, grid_info, bw_img, scales, valid_count)
% export_results_to_csv: 将分形维数计算结果导出到CSV文件
% 输入:
...
try
    ...
    % 将带有中文标题的表直接写入CSV文件
    writetable(data_table, csv_filename, 'WriteVariableNames', true, 'Encoding', 'UTF-8');
    
    fprintf('\n✅ 分形维数结果已导出到 CSV 文件: %s (带中文标题)\n', csv_filename);
    
catch ME
    % 如果上述方法失败，回退到不带中文标题的简单方法
    warning('FWZ:CSVExportFailed', '创建带中文标题的CSV文件失败: %s. 正在尝试不带标题的导出。', ME.message);
