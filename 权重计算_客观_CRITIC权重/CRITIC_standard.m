...
%standardCRITIC 使用标准CRITIC方法计算客观权重
%   该函数根据CRITIC（CRiteria Importance Through Intercriteria Correlation）
%   方法，通过分析指标的对比强度和冲突性来确定客观权重。
%
%   输入参数:
...
%
%   输出参数:
%   weights         - 一个 1 x n 的向量，包含每个指标的CRITIC权重。
...
%   [w, s] = standardCRITIC(data, indicator_types);
%   disp('计算出的权重为:');
%   disp(w);
%   disp('各样本的得分为:');
%   disp(s);
...
% --- 步骤 1: 数据标准化 (Min-Max Normalization) ---
...
conflict = sum(1 - R); % 对每一列求和，得到每个指标的冲突性

% --- 步骤 4: 计算信息承载量 (Information Content) ---
% 信息量 C 是对比强度和冲突性的乘积。
C = variability .* conflict;
